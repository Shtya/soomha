  ...
  return (
    <footer>
        ...
        {Hrefs.map( (href , i) => (
          <ul key={i}>
            ...
            <li className="bold"> {href.value} </li>
            {
              ...
              <ul key={idx}>
                  <li  > <Link className='hover:rtl:translate-x-[-10px] hover:ltr:translate-x-[10px] font-[400]   ' href={link.path} > {link.name} </Link> </li>
              </ul> 
            ))}
        ...
        <ul className=' flex flex-col gap-[10px] '>
          <li className='bold'> {t.raw('h3')[3]} </li>
          {
            data.map((e,i)=>(
              ...
              <Image className=' w-[33px] h-[33px] object-contain ' width={33} height={33} alt='' src={`/social/${e.social}.png`} /> </a>
            ))
          }
      ...
      <div className="  container mt-[40px] max-sm:!text-center flex  max-sm:!items-center max-sm:!gap-[10px] max-sm:!flex-col items-center !justify-between gap-[30px] "> 
        {t('rights')}  
        <ul className='flex !flex-row gap-[10px] items-center justify-center ' >
          <li> <Link className='underline font-[500] text-nowrap' href="/terms" > {t.raw("names")[15]} </Link> </li>
          <li> <Link className='underline font-[500] ' href="/privacy" > {t.raw("names")[16]} </Link> </li>
        </ul>
        </div>
    </footer>
  )
