...
export default function Alert({ setShow , show }) {
  ...
  return (
	<div className={` flex fixed  inset-0 z-[100000] bg-[#9d9d9d] bg-opacity-70 w-screen h-screen justify-center items-center max-sm:p-[30px]`} >
		<div className=' shadow-md p-[40px] max-sm:p-[20px] rounded-[10px] relative flex min-h-[320px] max-w-[500px] w-full bg-white items-center justify-between ' >
			
			<div className='flex flex-col h-full w-full items-center gap-[20px]' >
				...
				<div className='flex mt-[20px] items-center justify-between w-full gap-[30px] ' >
					...
					<Link href="" onClick={_=> setShow(!show)} className="rounded-[8px] hover:scale-[.94] duration-300 origin-center  h-[45px]  w-full bg-[#ff1c1c] text-white flex items-center justify-center cursor-pointer  "> {t[2]} </Link>
				</div>
			</div>
